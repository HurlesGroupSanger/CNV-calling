...
#   +bo 'box1;box2;box3'    # defines the order of boxplots, 'sort=1' to sort alphabetically, the default is leave as is
#
# CMDLINE
...
if lb!=None:
    ...
    for key_lb in key_lbs:
        ...
        #arr = key_lb.split(':')
        import re
        arr = re.split(r'(?<!\\):',key_lb)
        key = arr[0].replace('\\:',':')
        lb  = key
        if len(arr)>1: lb = arr[1].replace('\\:',':')
        lab.append(lb)
        keys.append(key)
        pos.append(ipos+1)
else:
...
if ann!=None:
    for x in ann:
        (coor,txt) = x.split(':')
        (cx,cy) = coor.split(',')
        ax.annotate(txt,xy=(float(cx),float(cy)),xycoords='axes fraction')
...
if style=='mine': adjust = {'left':0.15,'bottom':0.10,'right':0.95}
# adj: {adjust}
